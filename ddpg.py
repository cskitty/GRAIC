...
class DDPG(object):
    ...
    def __init__(self, args):
        ...
        self.scale_in = args['scale_in']
        self.epsilon = args['epsilon']
        
        self.actor = CUDA(Actor(self.state_dim, self.action_dim, self.scale_in))
        self.actor_target = CUDA(Actor(self.state_dim, self.action_dim, self.scale_in))
        self.actor_target.load_state_dict(self.actor.state_dict())
        self.actor_optimizer = optim.Adam(self.actor.parameters(), lr=self.actor_lr)

        self.critic = CUDA(Critic(self.state_dim, self.action_dim, self.scale_in))
        self.critic_target = CUDA(Critic(self.state_dim, self.action_dim, self.scale_in))
        self.critic_target.load_state_dict(self.critic.state_dict())
        self.critic_optimizer = optim.Adam(self.critic.parameters(), lr=self.critic_lr)

        # buffer for saving data
        self.replay_buffer = ReplayBuffer(self.memory_capacity, self.state_dim*2+self.action_dim+2)

    def store_transition(self, data):
        # preprocess the data before store t into buffer
        # [state_a, action, post_reward, next_state_a, done]
    ...
    def load_model(self, load_path):
        filepath = os.path.join(load_path, 'model.ddpg.'+str(self.model_id)+'.torch')
        if os.path.isfile(filepath):
            ...
            raise Exception('No DDPG model found!', filepath)
